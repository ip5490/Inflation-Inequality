...
file_name = ['logestimatesmerged.xlsx']
...
# Remove sectors without total effect estimates
...
plt.errorbar(estimatesmerged_df['Estimate Direct'], estimatesmerged_df['Sector'], 
             ...
             fmt='o', color='#FF5733', label='Estimate Direct Effect')
...
plt.errorbar(estimatesmerged_df['Estimate Indirect'], estimatesmerged_df['Sector'], 
             ...
             fmt='o', color='#377EB8', label='Estimate Production Network Effect')
...
for i, (_, row) in enumerate(estimatesmerged_df.iterrows()):    
    plt.plot([row['Direct Confi -'], row['Direct Confi -']], [i - 0.2, i + 0.2], color='#FF5733', linewidth=1, linestyle='-')
    plt.plot([row['Direct Confi +'], row['Direct Confi +']], [i - 0.2, i + 0.2], color='#FF5733', linewidth=1, linestyle='-')
    
    plt.plot([row['Indirect Confi -'], row['Indirect Confi -']], [i - 0.2, i + 0.2], color='#377EB8', linewidth=1, linestyle='-')
    plt.plot([row['Indirect Confi +'], row['Indirect Confi +']], [i - 0.2, i + 0.2], color='#377EB8', linewidth=1, linestyle='-')
...
plt.savefig(os.path.join(folder_path, 'estimates_plot.pdf'), bbox_inches='tight')
